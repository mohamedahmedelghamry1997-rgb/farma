    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans text-right" dir="rtl">
      ...
      <header className="bg-white/80 backdrop-blur-xl sticky top-0 z-50 py-4 border-b border-slate-100 px-4">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            {store.role && (
              <Badge variant="secondary" className="bg-slate-100 text-slate-600 border-none py-1.5 px-4 rounded-full font-black text-xs hidden md:flex">
            ...
            {store.role && store.role !== 'client' && (
              <Button variant="ghost" size="sm" onClick={() => { store.setRole(null); localStorage.removeItem('pb_role'); }} className="text-red-500 font-bold gap-2 hover:bg-red-50">
        ...
        {(!store.role || store.role === 'client') && (
          <div className="space-y-0 animate-slide-up">
            <div className="relative h-[80vh] flex items-center justify-center text-white overflow-hidden">
               <Image src="https://picsum.photos/seed/resort-hero/1920/1080" alt="Hero" fill className="object-cover brightness-[0.4]" priority />
               <div className="container mx-auto px-4 relative z-10 text-center space-y-8">
                  <Badge className="bg-primary/20 backdrop-blur-md text-primary-foreground border-primary/30 px-6 py-2 rounded-full font-black text-sm mb-4">أرقى وجهات الساحل والسخنة</Badge>
                  <h2 className="text-5xl md:text-8xl font-black leading-tight tracking-tighter">تجربة فندقية <br/><span className="text-primary">لا تُنسى</span></h2>
                  <p className="text-lg md:text-xl font-medium opacity-80 max-w-2xl mx-auto leading-relaxed">اكتشف مجموعتنا المختارة من الشاليهات الفاخرة المجهزة بأعلى معايير الجودة والرفاهية لضمان عطلة مثالية.</p>
                  <div className="flex flex-wrap justify-center gap-4 pt-4">
                    <Button className="bg-primary hover:bg-primary/90 text-white h-16 px-12 rounded-full text-xl font-black shadow-2xl shadow-primary/40 gap-3" onClick={() => document.getElementById('browse')?.scrollIntoView({ behavior: 'smooth' })}>
                      استعرض الشاليهات <ChevronLeft className="h-6 w-6" />
                    </Button>
                    <Button variant="outline" className="bg-white/10 backdrop-blur-md text-white border-white/20 h-16 px-12 rounded-full text-xl font-black hover:bg-white/20" onClick={() => document.getElementById('staff-gate')?.scrollIntoView({ behavior: 'smooth' })}>بوابة الشركاء</Button>
                  </div>
               </div>
            </div>
                  ...
                  <div className="flex gap-4 w-full md:w-auto">
                    <div className="relative flex-1 md:w-80">
                      ...
                      <Input 
                        ...
                        className="bg-white rounded-2xl h-14 pr-12 border-none shadow-sm focus:ring-2 focus:ring-primary/20" 
                      />
                    </div>
                  </div>
               </div>
            ...
            <div id="staff-gate" className="bg-slate-900 py-32 text-white relative overflow-hidden">
               ...
               <div className="container mx-auto px-4 text-center space-y-16 relative z-10">
                  <div className="space-y-4">
                    ...
                    <p className="text-slate-400 font-medium max-w-xl mx-auto">هذه المنطقة مخصصة لموظفي وشركاء منتجع فارما بيتش لإدارة الحجوزات والعمليات الميدانية والمالية.</p>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
               <Card className="p-8 rounded-[2.5rem] bg-white border-none shadow-sm flex items-center gap-6 group">
                  ...
                  <div className="text-right">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">الوحدات</p>
                    <p className="text-3xl font-black text-slate-800">{store.chalets.length}</p>
                  </div>
               </Card>
               <Card className="p-8 rounded-[2.5rem] bg-white border-none shadow-sm flex items-center gap-6 group">
                  <div className="bg-green-100 p-4 rounded-3xl text-green-600"><Activity className="h-8 w-8" /></div>
                  <div className="text-right">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">نزلاء حالياً</p>
                    <p className="text-3xl font-black text-slate-800">{stats.activeGuests}</p>
                  </div>
               </Card>
               <Card className="p-8 rounded-[2.5rem] bg-white border-none shadow-sm flex items-center gap-6 group">
                  <div className="bg-orange-100 p-4 rounded-3xl text-orange-600"><Wallet className="h-8 w-8" /></div>
                  <div className="text-right">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">بانتظار التحصيل</p>
                    <p className="text-3xl font-black text-slate-800">{stats.pendingFinance}</p>
                  </div>
               </Card>
               <Card className="p-8 rounded-[2.5rem] bg-white border-none shadow-sm flex items-center gap-6 group">
                  <div className="bg-purple-100 p-4 rounded-3xl text-purple-600"><TrendingUp className="h-8 w-8" /></div>
                  <div className="text-right">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">الإيرادات</p>
                    <p className="text-3xl font-black text-slate-800">{stats.totalRevenue.toLocaleString()} <span className="text-xs">ج.م</span></p>
                  </div>
               </Card>
            </div>
            ...
            <Tabs dir="rtl" defaultValue="bookings" className="w-full">
              ...
              <TabsContent value="bookings" className="space-y-6">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-2xl font-black text-slate-800">سجل العمليات المالية</h3>
                  <div className="flex gap-2">
                    <Button variant="outline" className="rounded-full border-slate-200 font-bold px-6">تحميل تقرير PDF</Button>
                  </div>
                </div>
                  ...
                  {filteredBookings.map(b => (
                    <Card key={b.id} className="p-6 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center gap-6 bg-white border-none shadow-sm hover:shadow-xl transition-all duration-300">
                      <div className="flex gap-6 items-center flex-1">
                         <div className={`p-5 rounded-[2rem] ${b.paymentStatus === 'verified' ? 'bg-green-50 text-green-500' : 'bg-orange-50 text-orange-500'}`}>
                         ...
                         <div className="text-right space-y-1">
                            ...
                            <div className="flex flex-wrap items-center gap-3 text-[10px] font-black text-slate-400">
                               ...
                               <span className="text-slate-500 flex items-center gap-1"><Clock className="h-3 w-3" /> {format(new Date(b.startDate), 'dd MMM')} - {format(new Date(b.endDate), 'dd MMM')}</span>
                            </div>
                         </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                         <div className="text-left ml-4 hidden md:block">
                            <p className="text-[10px] font-black text-slate-400 uppercase">المبلغ الإجمالي</p>
                            <p className="text-xl font-black text-primary">{b.totalAmount?.toLocaleString()} ج.م</p>
                         </div>
                         <Button variant="outline" className="rounded-2xl h-14 w-14 p-0 border-slate-100 bg-slate-50 hover:bg-slate-100" onClick={() => setReviewingBooking(b)}>
              ...
              <TabsContent value="chalets" className="space-y-8">
                <div className="flex justify-between items-center bg-white p-8 rounded-[2.5rem] shadow-sm">
                   <div>
                    <h3 className="text-2xl font-black text-slate-800">إدارة الأصول العقارية</h3>
                    <p className="text-slate-400 font-bold text-xs mt-1">تعديل وإضافة الوحدات المتاحة في المنظومة</p>
                   </div>
                    ...
                    <Card key={c.id} className="rounded-[3rem] overflow-hidden border-none shadow-sm bg-white group relative hover:shadow-2xl transition-all duration-500">
                       <div className="relative h-60">
                         ...
                         <div className="absolute top-4 left-4">
                           <Button size="icon" variant="destructive" className="h-10 w-10 rounded-full bg-white/10 backdrop-blur-md border border-white/20 hover:bg-red-500" onClick={() => store.deleteChalet(c.id)}>
                             <Trash2 className="h-5 w-5" />
                           </Button>
                         </div>
                       </div>
                       <div className="p-6 space-y-4">
                         <Button variant="secondary" className="w-full rounded-2xl h-12 font-black text-xs gap-2 bg-slate-50 text-slate-600 hover:bg-primary hover:text-white transition-colors" onClick={() => setViewingHistoryChalet(c)}>
                           <Clock className="h-4 w-4" /> سجل تشغيل الوحدة
                         </Button>
                       </div>
                    </Card>
                  ))}
              ...
              <TabsContent value="users" className="space-y-8">
                <div className="flex justify-between items-center bg-white p-8 rounded-[2.5rem] shadow-sm">
                   <h3 className="text-2xl font-black text-slate-800">فريق العمل والشركاء</h3>
                   {store.role === 'admin' && (
                  ...
                  {store.users.map(u => (
                    <Card key={u.id} className="p-8 rounded-[3rem] flex justify-between items-center bg-white border-none shadow-sm group hover:shadow-lg transition-all">
                      <div className="flex items-center gap-6">
                         <div className="bg-slate-50 p-6 rounded-[2.5rem] group-hover:bg-primary/5 transition-colors">
                            <Briefcase className="text-slate-400 h-10 w-10 group-hover:text-primary transition-colors" />
                         </div>
                         <div className="text-right">
                            <p className="font-black text-slate-800 text-2xl">{u.name}</p>
                            <p className="text-[10px] text-primary font-black uppercase tracking-widest mt-2 bg-primary/5 inline-block px-4 py-1.5 rounded-full border border-primary/10">
          ...
          <div className="container mx-auto px-4 py-8 space-y-10 animate-slide-up">
            <div className="bg-white p-12 rounded-[3.5rem] shadow-sm border border-slate-100 text-right flex flex-col md:flex-row justify-between items-center gap-6">
               <div className="space-y-2">
                 ...
                 <p className="text-slate-400 font-bold text-lg">لديك مهام تسليم وفحص لليوم. يرجى الدقة في تسجيل الحالة.</p>
               </div>
            ...
            <div className="grid grid-cols-1 gap-8">
              {filteredBookings.length === 0 ? (
                <Card className="p-32 text-center rounded-[3.5rem] border-dashed border-2 bg-white">
                  ...
                  <p className="text-slate-400 font-black text-xl">لا توجد مهام ميدانية حالياً</p>
                </Card>
              ) : (
                filteredBookings.map(b => (
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-10">
                      <div className="text-right space-y-6 flex-1">
                         <div className="flex items-center gap-6">
                           ...
                           <Badge className={`px-6 py-2 rounded-full font-black text-xs ${b.opStatus === 'waiting' ? 'bg-blue-100 text-blue-600' : 'bg-green-100 text-green-600'}`}>
                         ...
                         <div className="bg-slate-50/50 p-8 rounded-[2.5rem] grid grid-cols-1 md:grid-cols-2 gap-8 border border-slate-100">
                            <div className="space-y-1">
                              <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">بيانات العميل</p>
                              <p className="text-2xl font-black text-slate-900">{b.clientName}</p>
                            </div>
                            <div className="space-y-1">
                              <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">التواصل الفوري</p>
      ...
      <footer className="bg-slate-900 text-white py-24 mt-20">
         <div className="container mx-auto px-4 text-center space-y-10">
            ...
            <p className="text-slate-400 font-medium max-w-2xl mx-auto leading-loose text-lg">نظام الإدارة المركزي لضمان أقصى درجات الرقابة والجودة في التشغيل الفندقي. نحن نجمع بين الرفاهية للعملاء والدقة المطلقة للإدارة.</p>
      ...
      <Dialog open={!!reviewingBooking} onOpenChange={() => setReviewingBooking(null)}>
        <DialogContent className="rounded-[3.5rem] text-right p-0 overflow-hidden border-none max-w-xl">
           <div className="bg-primary p-10 text-white">
              ...
              <p className="opacity-70 mt-2 font-medium">مراجعة بيانات التحويل والمبالغ</p>
           </div>
           {reviewingBooking && (
             <div className="p-10 space-y-8">
                <div className="p-8 bg-blue-50 border border-blue-100 rounded-[2.5rem] space-y-4">
                   <div className="flex justify-between items-center border-b border-blue-200 pb-4">
                    <p className="text-[10px] text-blue-600 font-black uppercase tracking-widest">مرجع التحويل (Ref)</p>
                ...
                {reviewingBooking.conditionReport && (
                  <div className="p-6 bg-orange-50 rounded-[2rem] border border-orange-100 space-y-2">
                     <p className="text-[10px] text-orange-600 font-black uppercase tracking-widest flex items-center gap-2">تقرير فحص المغادرة <Activity className="h-3 w-3" /></p>
                     <p className="text-sm font-bold text-slate-700 leading-relaxed italic">"{reviewingBooking.conditionReport}"</p>
                     <p className="text-xs font-black text-orange-800 mt-4 bg-orange-200/50 inline-block px-4 py-1 rounded-full">تأمين مسترد: {reviewingBooking.securityDeposit} ج.م</p>
                  </div>
                )}
             </div>
           )}
      ...
      <Dialog open={!!opReportBooking} onOpenChange={() => setOpReportBooking(null)}>
        <DialogContent className="rounded-[3.5rem] text-right p-0 overflow-hidden border-none max-w-xl">
          <div className="bg-orange-600 p-10 text-white relative">
            ...
            <DialogDescription className="text-white/80 text-right mt-2 font-medium">يرجى فحص (التكييفات، العفش، النظافة) وتسجيل أي تلفيات قبل استعادة المفاتيح.</DialogDescription>
          ...
          <div className="p-10 space-y-8">
             <div className="space-y-3">
                <Label className="font-black text-slate-600 flex justify-end text-sm">ملاحظات الحالة الفنية</Label>
                <Textarea 
                  placeholder="مثال: التكييف يعمل بكفاءة، كسر بسيط في زجاج الطاولة..." 
                  className="rounded-[2rem] min-h-[160px] text-right bg-slate-50 border-none p-6 text-lg"
                  value={conditionText}
                  onChange={e => setConditionText(e.target.value)}
                />
             </div>
             <div className="space-y-3">
                <Label className="font-black text-slate-600 flex justify-end text-sm">مبلغ التأمين المسترد للعميل</Label>
                <Input 
                  type="number"
                  placeholder="0.00" 
                  className="rounded-2xl h-16 text-right bg-slate-50 border-none text-xl font-black px-6"
                  value={depositText}
          ...
          <DialogFooter className="p-10 pt-0 gap-4 flex flex-row-reverse">
             ...
             <Button variant="ghost" className="h-20 rounded-[2rem] font-bold text-slate-400" onClick={() => setOpReportBooking(null)}>إلغاء</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!viewingHistoryChalet} onOpenChange={() => setViewingHistoryChalet(null)}>
        <DialogContent className="rounded-[3.5rem] text-right max-w-3xl p-0 overflow-hidden border-none">
          <div className="bg-slate-900 p-10 text-white flex justify-between items-center">
            <div className="text-right">
              <DialogTitle className="text-3xl font-black text-right">سجل التشغيل التاريخي (The Log)</DialogTitle>
              <p className="opacity-60 font-bold mt-2">{viewingHistoryChalet?.name} - {viewingHistoryChalet?.location}</p>
            </div>
          ...
          <div className="p-10 space-y-6 max-h-[65vh] overflow-y-auto custom-scrollbar bg-slate-50">
             {store.bookings.filter(b => b.chaletId === viewingHistoryChalet?.id).length === 0 ? (
               <div className="text-center py-24 bg-white rounded-[3rem] border-dashed border-2 border-slate-200">
                 ...
                 <p className="text-slate-400 font-black text-xl">لا توجد سجلات تشغيل لهذه الوحدة بعد.</p>
               </div>
             ) : (
               store.bookings.filter(b => b.chaletId === viewingHistoryChalet?.id).map(b => (
                 <div key={b.id} className="p-8 bg-white rounded-[2.5rem] border border-slate-100 flex flex-col gap-5 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-center border-b border-slate-50 pb-4">
                       <div>
                        <p className="font-black text-slate-800 text-xl">{b.clientName}</p>
                        <p className="text-[10px] text-slate-400 font-bold">{b.phoneNumber}</p>
                       </div>
                       <Badge variant="outline" className="font-black px-4 py-1.5 rounded-full border-slate-200">{format(new Date(b.startDate), 'dd MMM yyyy')}</Badge>
                    </div>
                    <div className="grid grid-cols-2 gap-6 text-xs font-black text-slate-500">
                       ...
                       <span className="flex items-center gap-2 bg-slate-50 p-3 rounded-2xl"><Activity className="h-4 w-4 text-orange-500" /> تأمين مسترد: {b.securityDeposit || 0} ج.م</span>
                    </div>
                    {b.conditionReport && (
                      <div className="bg-slate-50 p-4 rounded-2xl border-r-4 border-primary/20">
                        <p className="text-xs font-bold text-slate-400 mb-1">تقرير الحالة عند المغادرة:</p>
                        <p className="text-sm font-bold italic text-slate-600 leading-relaxed">"{b.conditionReport}"</p>
                      </div>
                    )}
                 </div>
               ))
          ...
          <DialogFooter className="p-8 bg-white border-t border-slate-100">
             <Button variant="ghost" className="w-full h-14 rounded-2xl font-black text-slate-400 hover:bg-slate-50" onClick={() => setViewingHistoryChalet(null)}>إغلاق السجل</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
